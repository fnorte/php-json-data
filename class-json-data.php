...
abstract class JsonData
{
    ...
    protected  $file_name;
		...
		/**
		 * Description : Constructor, defining time_check and path.  
		 */
    public function __construct($time = false)
    {
        ...
        $this->path = __DIR__ . '/../data/'; // Path para armazenar o JSON.
        if (!is_dir($this->path)) {
	...
	public function mintxt( $txt ) {
		$txt = $this->remove_accents( $txt );
		$txt = strtoupper( $txt );
		$txt = str_replace( '  ', '', $txt );
